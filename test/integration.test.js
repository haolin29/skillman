...
test('full workflow: scan, install, list, uninstall', async () => {
  ...
  const originalInstalledFile = process.env.SKILLMAN_INSTALLED_FILE;
  process.env.SKILLMAN_INSTALLED_FILE = registryPath;
...
`);
  ...
  try {
    ...
    process.env.SKILLMAN_INSTALLED_FILE = originalInstalledFile;
